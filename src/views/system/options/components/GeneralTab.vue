<template>
  <n-form ref="formRef" :model="optionsGroup">
    <component
      :is="component[item.optionComponents]"
...
const optionsGroup = ref<OptionsGroup>({
  ...
  groupKey: '',
  optionList: [
    { id: '', optionName: '', optionComponents: '', optionKey: '', optionValue: '', optionLevel: '', status: 1 }
  ]
});
const formRef = ref<FormInst | null>(null);
onMounted(async () => {
  const { data } = await fetchSystemConfig('general_system_settings');
  if (data) {
